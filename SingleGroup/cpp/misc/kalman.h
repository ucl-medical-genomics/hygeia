...
namespace kalman
{
  ...
  /// (overload for multivariate case).
  double evaluateLogMarginalLikelihood(
    const arma::colvec& Fu, // intercept of transition equation
    const arma::mat& A, 
    const arma::mat& B, 
    const arma::colvec& Gu, // intercept of observation equation
    const arma::mat& C, 
    const arma::mat& D, 
    const arma::mat& m0, 
    const arma::mat& C0, 
  ...
  /// Runs a Kalman filter and returns the log-marginal likelihood
  /// (overload for multivariate case and without intercepts).
  double evaluateLogMarginalLikelihood(
    const arma::mat& A, 
    const arma::mat& B, 
    const arma::mat& C, 
  ...
  { 
    arma::colvec Fu = arma::zeros<arma::colvec>(A.n_rows);
    arma::colvec Gu = arma::zeros<arma::colvec>(C.n_rows);
    return evaluateLogMarginalLikelihood(Fu, A, B, Gu, C, D, m0, C0, y);
  }
  /// Runs a Kalman filter and returns the log-marginal likelihood
  /// (overload for multivariate case and with
  /// observations stored as a std::vector<arma::colvec>).
  double evaluateLogMarginalLikelihood(
    const arma::colvec& Fu, // intercept of transition equation
    const arma::mat& A, 
    const arma::mat& B, 
  ...
  )
  {
    ...
    for (unsigned int t=0; t<T; t++)
    {
      ...
      // Adding the incremental log-marginal likelihood
      logLikelihood += arma::as_scalar(gaussian::evaluateDensityMultivariate(y[t], mY, CY, false, true));
    }
    return logLikelihood;
  }
  /// Runs a Kalman filter and returns the log-marginal likelihood
  /// (overload for multivariate case, without intercepts and with
  /// observations stored as a std::vector<arma::colvec>).
  double evaluateLogMarginalLikelihood(
    const arma::mat& A, 
    const arma::mat& B, 
