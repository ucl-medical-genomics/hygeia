...
namespace gaussian
{
  ...
  // Normal random numbers: general covariance matrix // WARNING: check whether the cholesky factor needs to be transposed!
  ...
  // Samples from a truncated univariate normal distribution.
  double rtnorm(
    const double lower,
    const double upper,
    const double mean,
  ...
  /// Samples from a lower truncated univariate normal distribution.
  double rltnorm(
    const double lower,
    const double mean,
    const double sigma,
    bool is_sd = false)
  {
    ...
    {
      x = R::qnorm(arma::as_scalar(R::pnorm(lower, mean, sigma, true, false) + arma::randu(1) * 
            (1.0 - R::pnorm(lower, mean, sigma, true, false))),
            mean, sigma, true, false);
    }
    else 
    {
      x = R::qnorm(arma::as_scalar(R::pnorm(lower, mean, sqrt(sigma), true, false) + arma::randu(1) * 
            (1.0 - R::pnorm(lower, mean, sqrt(sigma), true, false))),
            mean, sqrt(sigma), true, false);
    }
    
    return(x);
  }
  /// Evaluates the density of a truncated univariate normal distribution.
  double dtnorm(
    const double x,
    const double lower,
    const double upper,
  ...
  /// Returns the density of a lower-truncated univariate normal distribution.
  double dltnorm(
    const double x,
    const double lower,
    const double mean,
    ...
    bool logd = false)
  {
    ...
    {
      logDensity = R::dnorm(x, mean, sigma, 1) - 
                  log(1.0 - R::pnorm(lower, mean, sigma, 1, 0));
    }
    else 
    {
      logDensity = R::dnorm(x, mean, sqrt(sigma), 1) - 
                  log(1.0 - R::pnorm(lower, mean, sqrt(sigma), 1, 0));
    }
