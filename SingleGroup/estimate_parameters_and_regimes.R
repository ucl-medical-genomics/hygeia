...
aux <- get_known_parameters(
  ...
  mu = mu, 
  sigma = sigma,
  is_kappa_fixed = args$is_kappa_fixed, 
  kappa = kappa
)
...
if (args$estimate_parameters) { # i.e., if we need to estimate the model parameters from the data
...
csv_file_genomic_positions  <- file.path(args$inputdir, "genomic_positions.csv")
csv_file_n_total_reads      <- file.path(args$inputdir, "n_total_reads.csv")
csv_file_n_methylated_reads <- file.path(args$inputdir, "n_methylated_reads.csv")
...
aux <- runOnlineCombinedInferenceCpp(
  ...
  args$n_particles, # number particles used by the SMC algorithm
  ...
  args$estimate_regime_probabilities, # determines if we should estimate the expectations of the regimes under the marginal smoothing distributions
  args$epsilon, # tuning parameter for the online-smoothing algorithm
  args$estimate_parameters, # determines if the model parameters should be updated throughout the algorithm (otherwise they are fixed at their initial values)
  args$normalise_gradients, # should the gradients be normalised according to their $L_1$ norm?
  args$use_adam, # should the ADAM optimiser be used (otherwise it is plain stochastic gradient ascent)
  args$n_steps_without_parameter_update, # number of SMC steps without parameter updates
  args$learning_rate_exponent, # the exponent governing the learning-rate decay
  args$learning_rate_factor # the factor governing the learning-rate decay
)


# store regime estimates
if (args$estimate_regime_probabilities) {
  ...
  aux$regimeProbabilityEstimates %>%
    ...
    readr::write_csv(
      file = file.path(args$outputdir, "regime_probabilities.csv")
    )
}
...
if (args$estimate_parameters) {
  ...
  aux$thetaEstimates %>%
    ...
    write_csv(file = file.path(args$outputdir, "theta_trace.csv"))
  ...
  final_parameter_estimates$p %>%
    write_p_to_csv_file(dir = args$outputdir)
  
  final_parameter_estimates$omega %>%
    write_omega_to_csv_file(dir = args$outputdir)
  
  if (!get_is_kappa_fixed(vartheta)) {
    final_parameter_estimates$kappa %>%
      write_kappa_to_csv_file(dir = args$outputdir)
  }
